...
extern "C" void sort3_cassioneri_14(int* buffer) {
  __asm volatile("# LLVM-MCA-BEGIN sort3_cassioneri_14":::"memory");
  int a, b, c;
  asm volatile (
    "mov (%[p]), %[a]           \n\t" // int a = buffer[0];
    "mov 4(%[p]), %[b]          \n\t" // int b = buffer[1];
...
extern "C" void sort3_cassioneri_15(int* buffer) {
  int a, b, c;
  int64_t i, j;
  __asm volatile("# LLVM-MCA-BEGIN sort3_cassioneri_15":::"memory");
  asm volatile (
    "mov (%[p]), %[a]           \n\t" // int a = buffer[0];
    "mov 4(%[p]), %[b]          \n\t" // int b = buffer[1];
    "mov 8(%[p]), %[c]          \n\t" // int c = buffer[2];
...
extern "C" void sort3_cassioneri_15_v2(int* buffer) {
  int a, b, c;
  int64_t i, j;
  __asm volatile("# LLVM-MCA-BEGIN sort3_cassioneri_15_v2":::"memory");
  asm volatile (
    "mov (%[p]), %[a]         \n\t" // int a = buffer[0];
    "mov 4(%[p]), %[b]        \n\t" // int b = buffer[1];
    "mov 8(%[p]), %[c]        \n\t" // int c = buffer[2];
...
extern "C" void sort3_cassioneri_faster(int* buffer) {
  ...
  __asm volatile("# LLVM-MCA-BEGIN sort3_cassioneri_faster":::"memory");
  int a = buffer[0];
  int b = buffer[1];
  int c = buffer[2];
